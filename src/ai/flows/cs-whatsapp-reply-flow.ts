...
// Hardcoded promptZoya (versi terakhir yang sudah diperbaiki)
const promptZoya = `
...
const replyPromptSimplified = ai.definePrompt({
  ...
  input: { schema: WhatsAppReplyInputSchema }, // Skema input masih termasuk mainPromptString, tapi tidak dipakai di sini
...
export async function generateWhatsAppReply(input: Omit<WhatsAppReplyInput, 'mainPromptString'>): Promise<WhatsAppReplyOutput> {
  ...
  try {
    ...
    if (settingsSnap.exists() && settingsSnap.data()?.mainPrompt && settingsSnap.data()?.mainPrompt.trim() !== "") {
      ...
      console.log("[CS-FLOW] Berhasil memuat prompt utama dari Firestore (meskipun tidak akan dipakai oleh flow saat ini).");
    } else {
      console.warn("[CS-FLOW] Gagal memuat prompt utama dari Firestore atau field mainPrompt kosong/tidak ada. Mencoba fallback ke default (meskipun tidak akan dipakai oleh flow saat ini).");
      if (DEFAULT_AI_SETTINGS.mainPrompt && DEFAULT_AI_SETTINGS.mainPrompt.trim() !== "") {
        promptFromSettings = DEFAULT_AI_SETTINGS.mainPrompt;
        console.log("[CS-FLOW] Menggunakan prompt default dari DEFAULT_AI_SETTINGS (meskipun tidak akan dipakai oleh flow saat ini).");
      } else {
        console.error("[CS-FLOW] KRITIS: Prompt utama dari Firestore KOSONG DAN prompt default juga KOSONG. Menggunakan fallback minimal darurat (meskipun tidak akan dipakai oleh flow saat ini).");
    ...
    console.warn("[CS-FLOW] Menggunakan prompt default karena gagal fetch atau prompt Firestore kosong (meskipun tidak akan dipakai oleh flow saat ini).");
    if (DEFAULT_AI_SETTINGS.mainPrompt && DEFAULT_AI_SETTINGS.mainPrompt.trim() !== "") {
        promptFromSettings = DEFAULT_AI_SETTINGS.mainPrompt;
        console.log("[CS-FLOW] Menggunakan prompt default dari DEFAULT_AI_SETTINGS setelah error fetch (meskipun tidak akan dipakai oleh flow saat ini).");
    } else {
        console.error("[CS-FLOW] KRITIS: Gagal fetch Firestore DAN prompt default juga KOSONG. Menggunakan fallback minimal darurat (meskipun tidak akan dipakai oleh flow saat ini).");
        promptFromSettings = "Anda adalah asisten AI. Tolong jawab pertanyaan pengguna: {{{customerMessage}}}"; // Darurat fallback
    }
  }

  if (!promptFromSettings || promptFromSettings.trim() === "") {
      console.error("[CS-FLOW] KRITIS FINAL CHECK: Prompt utama tetap kosong setelah semua fallback. Ini tidak seharusnya terjadi. Menggunakan fallback minimal darurat terakhir (meskipun tidak akan dipakai oleh flow saat ini).");
  ...
  const flowInput: WhatsAppReplyInput = {
    ...input,
    mainPromptString: promptFromSettings, // Tetap tambahkan, meskipun flow akan pakai versi hardcode
