  ...
  async (input) => {
    ...
    try {
      ...
      if (foundClientData) {
        ...
        if (Array.isArray(foundClientData.motorcycles)) { // Safely check if motorcycles is an array
            mappedMotorcycles = foundClientData.motorcycles.map(m => ({ name: m.name, licensePlate: m.licensePlate }));
        } else if (foundClientData.motorcycles) { // Log if it exists but is not an array
