        ...
        if (response.ok && responseData.success && responseData.reply && responseData.reply.suggestedReply) {
            ...
            if (aiReplyText.length > MAX_MESSAGE_LENGTH) {
                ...
                while (remainingText.length > 0) {
                    ...
                    // 1. Try to split at the last newline within MAX_MESSAGE_LENGTH
                    splitPoint = tempChunk.lastIndexOf('\\n');
                    if (splitPoint !== -1 && splitPoint > MAX_MESSAGE_LENGTH * 0.6) { // Prefer newline if it's reasonably far
                        // Keep splitPoint
                    } else {
                        // 2. If no suitable newline, try to split at the last sentence end (.!?)
                        let lastSentenceEndPoint = -1;
                        let currentSearchIndex = 0;
                        let tempSearchChunk = tempChunk;

                        // Find the last sentence end within the tempChunk
                        for (let i = tempSearchChunk.length -1; i >=0; i--) {
                            if (/[.!?]/.test(tempSearchChunk[i]) && (i + 1 < tempSearchChunk.length && tempSearchChunk[i+1] === ' ')) {
                                lastSentenceEndPoint = i + 1; // Include the punctuation
                                break;
                            }
                        }

                        if (lastSentenceEndPoint > MAX_MESSAGE_LENGTH * 0.5) { // Prefer sentence split if it's reasonably long
                            splitPoint = lastSentenceEndPoint;
                        } else {
                            // 3. If still no good split point, try last space
                            splitPoint = tempChunk.lastIndexOf(' ');
                            if (splitPoint === -1 || splitPoint < MAX_MESSAGE_LENGTH * 0.3) { // If no space or space is too early
                                splitPoint = MAX_MESSAGE_LENGTH; // Hard split
                            }
                        }
                    }
                ...
                for (let i = 0; i < messageChunks.length; i++) {
                    ...
                    if (chunkToSend.length > 0) {
                        await client.sendMessage(msg.from, chunkToSend);
