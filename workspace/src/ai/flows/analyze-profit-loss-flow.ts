...
import type { AiConfiguration, SafetySetting } from '@/types/aiSettings';
import { AiConfigurationSchema, DEFAULT_SAFETY_SETTINGS } from '@/types/aiSettings';
...
// Schema for prompt input, now including custom instruction
const PromptInputSchema = AnalyzeProfitLossInputSchema.extend({
  profitLossCustomInstruction: z.string().optional(),
});
...
{{#if profitLossCustomInstruction}}

Instruksi Tambahan dari Pengguna untuk Analisa Ini:
...
const analyzeProfitLossFlow = ai.defineFlow(
  ...
  async (input) => {
    let aiConfig: AiConfiguration = AiConfigurationSchema.parse({}); // Get defaults
    let safetySettingsToUse: SafetySetting[] = DEFAULT_SAFETY_SETTINGS;
    let customInstructionToUse: string | undefined = undefined;

    try {
      const settingsDocRef = doc(db, 'appSettings', 'aiConfiguration');
      const docSnap = await getDoc(settingsDocRef);
      if (docSnap.exists()) {
        aiConfig = AiConfigurationSchema.parse(docSnap.data());
         console.log("AI Configuration loaded for P&L:", aiConfig);
      } else {
        console.log("AI Configuration document not found for P&L, using defaults.");
      }
      safetySettingsToUse = aiConfig.globalSafetySettings;
      customInstructionToUse = aiConfig.profitLossCustomInstruction;
    } catch (error) {
      console.error("Error fetching AI configuration for P&L, using defaults:", error);
    }
    
    const promptInputData = {
      ...input,
      profitLossCustomInstruction: customInstructionToUse,
    };

    const {output} = await analyzePrompt(promptInputData, { config: { safetySettings: safetySettingsToUse } });
    if (!output) {
