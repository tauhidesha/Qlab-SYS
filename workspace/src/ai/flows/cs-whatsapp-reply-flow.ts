...
const WhatsAppReplyInputSchema = z.object({
  ...
  // Custom instruction will be fetched by the flow, not passed in directly
});
...
const PromptInputSchema = WhatsAppReplyInputSchema.extend({
  csAssistantCustomInstruction: z.string().optional(),
});
...
{{#if csAssistantCustomInstruction}}
...
const whatsAppReplyFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        // Parse with schema to get defaults for missing fields
        aiConfig = AiConfigurationSchema.parse(docSnap.data());
        console.log("AI Configuration loaded:", aiConfig);
      } else {
        console.log("AI Configuration document not found, using defaults.");
      }
      safetySettingsToUse = aiConfig.globalSafetySettings;
      customInstructionToUse = aiConfig.csAssistantCustomInstruction;
    } catch (error) {
    ...
    const promptInputData = {
      ...input,
      csAssistantCustomInstruction: customInstructionToUse,
    };
    
    const {output} = await replyPrompt(promptInputData, { config: { safetySettings: safetySettingsToUse } });
    if (!output) {
